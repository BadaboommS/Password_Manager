...
import { writeUserData, readUserData, encryptData, decryptData, isEncryptionAvailable, initMkdir, getStorageFiles } from './services/main.service';
...
ipcMain.on("writeUserPwdData", (e, pwdData: PwdArray) => {
  try{
    if(isEncryptionAvailable()){
      const stringPwdData = pwdData.map((obj) => JSON.stringify(obj)).join('-|-');
      const encryptedData = encryptData(stringPwdData);
      writeUserData(encryptedData);
    }
  }catch(err){
